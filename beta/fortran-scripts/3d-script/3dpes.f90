...
contains
  function Fourier(Fp, X, Y)
    ...
    Fourier = Fp(1) + Fp(2) * (cos(2 * pi * X) + cos(2 * pi * Y)) &
         + Fp(3) * (cos(2 * pi * (X + Y)) + cos(2 * pi * (X - Y))) &
         + Fp(4) * (cos(4 * pi * X) + cos(4 * pi * Y)) &
         + Fp(5) * (cos(2 * pi * (2 * X + Y)) + cos(2 * pi * (X + 2 * Y)) &
         + cos(2 * pi * (2 * X - Y)) + cos(2 * pi * (X - 2 * Y))) &
         + Fp(6) * (cos(4 * pi * (X + Y)) + cos(4 * pi * (X - Y)))
